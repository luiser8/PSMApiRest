...
namespace PSMApiRest.DAL
{
    public class UsersDAL
    {
            ...
            if (dbCon.ErrorEstatus)
            {
                if (dt.Rows.Count != 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        users.NombreRol = Convert.ToString(dt.Rows[i]["NombreRol"]);
        ...
        public List<Users> UpdateUsuarios(Users user)
        {
            ...
            Parametros.Add("@RolId", user.RolId != 0 ? user.RolId : 0);
            Parametros.Add("@Usuario", user.Usuario != "" ? user.Usuario : null);
            Parametros.Add("@Cedula", user.Cedula != 0 ? user.Cedula : 0);
            Parametros.Add("@Nombres", user.Nombres != "" ? user.Nombres : null);
            Parametros.Add("@Apellidos", user.Apellidos != "" ? user.Apellidos : null);
            Parametros.Add("@Bloqueado", user.Bloqueado);
            Parametros.Add("@Estado", user.Estado);
            Parametros.Add("@Contrasena", user.Contrasena != null ? MD5.GetMD5(user.Contrasena) : null);
